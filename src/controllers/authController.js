class AuthController {
    ...
    async signup(req, res) {
        ...
        try {
            const { userId } = await this.authService.signup(email, password, { username, birthdate, skillsWanted, skillsOffered });
            return res.status(201).json({ success: true, message: "Account Created Successfully.", userId });
        } catch (err) {
